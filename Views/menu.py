        ...
        while True:
            ...
            if choice == 1:
                ...
                if tier.lower() == "standard": 
                    m = StandardMembership(m_id, name, phone)
                elif tier.lower() == "premium": 
                    m = PremiumMembership(m_id, name, phone)
                elif tier.lower() == "vip": 
                    m = VIPMembership(m_id, name, phone)
                else:
                    print("❌ Hạng thẻ sai quy định!")
                ...
                print("\n--- DANH SÁCH HỘI VIÊN ---")
                members = self.mem_service.get_all_members()
                if not members:
                ...
                results = self.mem_service.search_member(kw)
                ...
                csv_opt = input("\nBạn có muốn xuất báo cáo thành file CSV không? (y/n): ")
                if csv_opt.lower() == 'y':
                ...
                print("👋 Hệ thống đang thoát...")
                break
                
            else:
                print("❌ Lựa chọn không hợp lệ! Vui lòng chọn trong khoảng (0-9).")
                input("\n👉 Nhấn Enter để quay lại Menu...")